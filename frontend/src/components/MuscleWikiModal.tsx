...
// Built-in Interactive SVG Muscle Anatomy Diagram (Zero external network dependencies)
const MuscleAnatomySVG: React.FC<{ muscle: string }> = ({ muscle }) => {
    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
      <svg viewBox="0 0 200 220" style={{ width: '100%', height: '160px', filter: 'drop-shadow(0 4px 10px rgba(0,0,0,0.5))' }}>
        <defs>
          ...
          <linearGradient id="backG" x1="0" y1="0" x2="1" y2="1">
        ...
        {/* Neck / Traps */}
        <path d="M 90 34 L 110 34 L 120 44 L 80 44 Z" fill={isBack ? 'url(#backG)' : baseColor} stroke={bodyOutline} strokeWidth="1" />
        ...
        {/* Chest (Pectorals) */}
        <path d="M 80 46 C 88 46, 98 48, 98 64 C 88 64, 76 60, 80 46 Z" fill={isChest ? highlightColor : (isBack ? baseColor : baseColor)} stroke={bodyOutline} strokeWidth="1" />
        <path d="M 120 46 C 112 46, 102 48, 102 64 C 112 64, 124 60, 120 46 Z" fill={isChest ? highlightColor : (isBack ? baseColor : baseColor)} stroke={bodyOutline} strokeWidth="1" />
        ...
        {/* Core / Abs & Obliques */}
        <path d="M 86 68 L 114 68 L 110 110 L 90 110 Z" fill={isAbs ? highlightColor : (isBack ? 'url(#backG)' : baseColor)} stroke={bodyOutline} strokeWidth="1" />
        {isAbs && (
          <>
...
export const MuscleWikiModal: React.FC<MuscleWikiModalProps> = ({
  ...
  // Smart Arabic Instruction Generator (when instructions are in English or missing)
  const getSmartArabicInstructions = (_exName: string, muscleEn: string, _eqEn: string): string[] => {
    ...
    if (m.includes('shoulder') || m.includes('trap')) {
      return [
        ...
        'توقف لأجزاء من الثانية في قمة الحركة للتركيز على استهداف الرأس المخصص من الدالية (الكتف).',
  ...
  // Determine Arabic instructions
  let stepsList: string[] = [];
  const rawInstructions = exercise.instructions_ar || exercise.description_ar;
  const isRawEnglish = rawInstructions && /^[a-zA-Z0-9\s.,'()\-]+$/.test(rawInstructions.substring(0, 50));

  if (isAr) {
    if (rawInstructions && !isRawEnglish && rawInstructions.trim().length > 10) {
      stepsList = rawInstructions
        .split(/[.\n]+/)
        .map((s: string) => s.trim())
        .filter((s: string) => s.length > 3);
    } else {
      stepsList = getSmartArabicInstructions(name, exercise.muscle_en || '', exercise.equipment_en || '');
    }
  } else {
    const engInstructions = exercise.instructions_en || exercise.instructions_ar || exercise.description_en;
    stepsList = engInstructions
      ? engInstructions
          .split(/[.\n]+/)
          .map((s: string) => s.trim())
          .filter((s: string) => s.length > 3)
  ...
  const isMistakeEnglish = rawMistakes && /^[a-zA-Z0-9\s.,'()\-]+$/.test(rawMistakes.substring(0, 30));

  if (isAr) {
    if (rawMistakes && !isMistakeEnglish && rawMistakes.trim().length > 5) {
      mistakesList = rawMistakes
        .split(/[\n]+/)
        .map((m: string) => m.trim())
        .filter((m: string) => m.length > 3);
    } else {
      mistakesList = getSmartArabicMistakes(exercise.muscle_en || '');
    }
  } else {
    const engMistakes = exercise.common_mistakes_en || exercise.common_mistakes_ar;
    mistakesList = engMistakes
      ? engMistakes
          .split(/[\n]+/)
          .map((m: string) => m.trim())
          .filter((m: string) => m.length > 3)
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '20px' }}>
          ...
          <div style={{ position: 'relative', height: '220px', background: 'rgba(15, 23, 42, 0.6)', borderRadius: '18px', overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(0, 210, 255, 0.2)', padding: '10px' }}>
            ...
            <MuscleAnatomySVG muscle={exercise.muscle_en || exercise.targetMuscle || 'Chest'} />
          </div>
        </div>
